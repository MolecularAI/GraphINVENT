...
class GraphGenerator:
    ...
    def sample(self) -> Tuple[list, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        -------
            ...
            generated_nlls (torch.Tensor) : Sampled NLLs per action for generated graphs.
            final_nlls (torch.Tensor) : Final total NLLs (sum) for generated graphs.
            properly_terminated (torch.Tensor) : Binary vector indicating if graphs were properly
              terminated (or not).
        """
    ...
    def allocate_graph_tensors(self) -> None:
        ...
        self.generated_n_nodes = torch.zeros(n_allocate, dtype=torch.int8, device=constants.device)

        # placeholder for sampled NLL per action for all graphs
        self.nlls = torch.zeros(nlls_shape, device=constants.device)

        # placeholder for sampled NLLs per action for all finished graphs
        self.generated_nlls = torch.zeros((n_allocate, *nlls_shape[1:]), device=constants.device)
    ...
    def apply_actions(self, add : Tuple[torch.Tensor, ...],
                      ...
                      nlls_sampled : torch.Tensor) -> None:
        """
        ...
        Updates the following tensors:
            ...
            self.n_nodes (torch.Tensor) : Updated number of nodes per graph (batch).
            self.nlls (torch.Tensor) : Updated sampled NLL per action for graphs (batch).
        ...
        def _add_nodes(add : Tuple[torch.Tensor, ...], generation_round : int,
            ...
            ----
                add (tuple) : Indices for "add" actions sampled for batch of graphs.
                generation_round (int) : Indicates current generation round.
                nlls_sampled (torch.Tensor) : NLL per action sampled for the most
    ...
    def copy_terminated_graphs(self, terminate_idc : torch.Tensor, n_graphs_generated : int,
                               generation_round : int, nlls_sampled : torch.Tensor) -> int:
        """
        ...
        Args:
        ----
            terminate_idc (torch.Tensor) : Indices for graphs that will terminate this round.
            ...
            nlls_sampled (torch.Tensor) : NLLs for the newest sampled action for each graph
    ...
    def initialize_graph_batch(self) -> None:
        ...
        generation process. Creates the following:
            ...
            self.n_nodes (torch.Tensor) : Number of nodes per graph in (batch), currently all 0.

        Also, creates a dummy "non-empty" graph at index 0, so that the models do not
        freak out when they receive entirely zero torch tensors as input (haven't found
        ...
        # initialize tensors
        self.nodes = torch.zeros(node_shape, dtype=torch.float32, device=constants.device)
        self.edges = torch.zeros(edge_shape, dtype=torch.float32, device=constants.device)
        self.n_nodes = torch.zeros(n_nodes_shape, dtype=torch.int8, device=constants.device)
    ...
    def reset_graphs(self, idc : int) -> None:
        ...
        invalid actions (indicated by `idc`). Updates the following:
            ...
            self.n_nodes_reset (torch.Tensor) : Reset number of nodes per graph (batch).
            self.nlls_reset (torch.Tensor) : Reset sampled NLL per action for graphs (batch).
        ...
        if len(idc) > 0:
            ...
            self.n_nodes[idc] = torch.zeros((len(idc), *n_nodes_shape[1:]),
                                            dtype=torch.int8,
                                            device=constants.device)
            self.nlls[idc] = torch.zeros((len(idc), *nlls_shape[1:]),
                                         dtype=torch.float32,
                                         device=constants.device)
    ...
    def get_actions(self, apds : torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        """
        def _reshape_apd(apds : torch.Tensor, batch_size : int) -> Tuple[torch.Tensor, ...]:
            """
        ...
        def _sample_apd(apds : torch.Tensor, batch_size : int) -> Tuple[torch.Tensor, ...]:
            """
    ...
    def graph_to_graph(self, idx : int) -> GenerationGraph:
        ...
        """
        def _features_to_atom(node_idx : int, node_features : torch.Tensor) -> rdkit.Chem.Atom:
            """
            ...
            if constants.use_chirality:
                cip_code_idx = (
                    ...
                    - constants.n_atom_types
                    - constants.n_formal_charge
                    - (not constants.use_explicit_H and not constants.ignore_H) * constants.n_imp_H
                )
        ...
        def _graph_to_mol(node_features : torch.Tensor, edge_features : torch.Tensor,
            ...
            edge_mask = torch.triu(
                torch.ones((n_max_nodes, n_max_nodes), device=constants.device), diagonal=1
            )
